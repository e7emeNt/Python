...
class Ship:
    ...
    def update(self):
        self.pos[0] += self.vel[0]*0.3
        self.pos[0] = self.pos[0] % WIDTH
        self.pos[1] += self.vel[1]*0.3
        ...
        if self.thrust:
            ...
            self.vel[0] = (1 - friction) * self.vel[0] + foward[0]
            self.vel[1] = (1 - friction) * self.vel[1] + foward[1]
        else:
            ship_thrust_sound.rewind()
            
    
    def change_ang(self, vel):
        self.angle_vel = vel
    ...
    def shoot(self):
        ...
        missile_vel = [self.vel[0] + 1.3 * forward[0],
                       self.vel[1] + 1.3 * forward[1]]
        return Sprite(missile_pos, missile_vel, self.angle, 0, missile_image,
...
def draw(canvas):
    ...
    canvas.draw_text("Lives :"+" "+str(lives), (30, 50), 25, "white", "sans-serif")
    canvas.draw_text("Score :"+" "+str(score), (670, 50), 25, "white", "sans-serif")


def keydown(key):
    global a_missile
    if key == simplegui.KEY_MAP["left"]:
        my_ship.change_ang(-0.1)
